...
This uses `parsnip::svm_rb()` and sets the `parsnip::engine` to
`kernlab`.
...
#>   before normalizing.
#> Error in fit_xy(spec, x = mold$predictors, y = mold$outcomes, case_weights = case_weights,     control = control_parsnip): Please install the kernlab package to use this engine.

ts_auto_rbf$recipe_info
#> Error: object 'ts_auto_rbf' not found
# }
```
